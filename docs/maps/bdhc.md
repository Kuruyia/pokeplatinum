...
This document aims to provide an overview of the BDHC subsystem, which is used to provide height information on maps.
...
It is basically Pokémon's way of adding height to an otherwise 2D engine (at
least, from the perspective of the map).

BDHC data is stored alongside each map data.
...
This section is targeted at people who are interested in the math necessary to
calculate the height of a point on a plate. For instance, this can be useful for
...
This section is targeted at people who are interested in the math necessary to
calculate the unit normal vector and constant of a plate. For instance, this can be
useful for implementing a map editor.
...
Instead of having to check every plate in the map to find the ones that contains
