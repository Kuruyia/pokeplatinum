...
similar 2D grid system that were used in previous games, on which the terrain attributes
dictates the collision and behavior of each tile, and on which map objects (such
as the player) are constrained, but they also have 3D elements, such as the map
model which is used to render the map, and the BDHC subsystem which is used to add
height and height variations to the terrain.
...
- attributes for each tile of the map, which dictates whether a tile can be
  walked on (collision data), and how the tile behaves when walked on: is it
  tall grass, water, a trash can, etc.
...
If you can make sense of it, here are the areas of the overworld map matrix:

// TODO: Put the illustration here
...
a warp, which allows the game to load a different set of resources.
