...
static OverworldAnimManagerList *OverworldAnimManagerList_Alloc(int heapID);
static void OverworldAnimManagerList_FreeSelf(OverworldAnimManagerList *managerList);
static OverworldAnimManager *OverworldAnimManager_AllocMultiple(int heapID, int count);
...
OverworldAnimManagerList *OverworldAnimManagerList_New(int heapID, int managerCount)
{
...
OverworldAnimManager *OverworldAnimManagerList_InitManagerEx(OverworldAnimManagerList *managerList, const OverworldAnimManagerExtendedFuncs *funcs, const VecFx32 *initialPos, int id, const void *userData, int sysTaskPriority)
{
    ...
    do {
        if (OverworldAnimManager_IsActive(iter) == FALSE) {
            break;
        }
    ...
    OverworldAnimManager_ResetFuncsContext(iter, funcs->ctxSize);
    OverworldAnimManager_SetInitFunc(iter, funcs->initFunc);
    OverworldAnimManager_SetExitFunc(iter, funcs->exitFunc);
    OverworldAnimManager_SetTickFunc(iter, funcs->tickFunc);
    OverworldAnimManager_SetPostTickFunc(iter, funcs->postTickFunc);
    ...
    if (OverworldAnimManager_CallInitFunc(iter) == FALSE) {
...
OverworldAnimManager *OverworldAnimManagerList_InitManager(OverworldAnimManagerList *managerList, const OverworldAnimManagerFuncs *funcs, const VecFx32 *initialPos, int id, const void *userData, int sysTaskPriority)
{
    ...
    *(OverworldAnimManagerFuncs *)&funcsEx = *funcs;
...
void OverworldAnimManager_Finish(OverworldAnimManager *manager)
{
    ...
    if (OverworldAnimManager_IsActive(manager) == FALSE) {
        return;
    }
...
void OverworldAnimManagerList_PostTick(OverworldAnimManagerList *managerList)
{
    int i = OverworldAnimManagerList_GetManagerCount(managerList);
    OverworldAnimManager *iter = OverworldAnimManagerList_GetManagers(managerList);
    ...
    do {
        ...
        if (flags == (OVERWORLD_ANIM_MANAGER_ACTIVE_FLAG | OVERWORLD_ANIM_MANAGER_TICKING_FLAG)) {
            OverworldAnimManager_CallPostTickFunc(iter);
        }
...
static void OverworldAnimManager_Reset(OverworldAnimManager *manager)
{
    ...
    OverworldAnimManager_SetPostTickFunc(manager, OverworldAnimManager_DummyPostTickFunc);
...
void OverworldAnimManager_SetPostTickFunc(OverworldAnimManager *manager, OverworldAnimManager_PostTickFunc postTickFunc)
{
    manager->postTickFunc = postTickFunc;
}

void OverworldAnimManager_CallPostTickFunc(OverworldAnimManager *manager)
{
    void *ctx = OverworldAnimManager_GetFuncsContext(manager);
    manager->postTickFunc(manager, ctx);
}
...
void OverworldAnimManager_DummyPostTickFunc(OverworldAnimManager *manager, void *ctx)
{
    return;
}
