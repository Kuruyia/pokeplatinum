    ...
    case 1: {
        ...
        if (!v1->unk_02) {
            if (sub_0206CB8C(&v1->unk_10, &v1->unk_14, &v1->unk_02)) {
                ...
                MapPropOneShotAnimationManager_PlayAnimation(fieldSystem->mapPropOneShotAnimationManager, 2, 0);
                Sound_PlayEffect(1758);
            }
        } else {
            if ((ov5_021D42F0(fieldSystem->mapPropOneShotAnimationManager, 1)) && (ov5_021D42F0(fieldSystem->mapPropOneShotAnimationManager, 2)) && v2) {
                v1->unk_00 = 2;
            }
        }
    } break;
...
static void sub_0206CBA0(FieldSystem *fieldSystem)
{
    ...
    BOOL v1;
    int v2[2] = {
        MAP_PROP_MODEL_ID_CANALAVE_BRIDGE_LEFT,
        MAP_PROP_MODEL_ID_CANALAVE_BRIDGE_RIGHT
    };
    int v3[2] = { 1, 2 };
    ...
    for (v0 = 0; v0 < 2; v0++) {
        ...
        if (v1) {
            ...
            ov5_021D41C8(fieldSystem->mapPropAnimationManager, fieldSystem->mapPropOneShotAnimationManager, v3[v0], v2[v0], v7, v4, AreaDataManager_GetMapPropTexture(fieldSystem->areaDataManager), 1, 1, 0);
        } else {
            GF_ASSERT(FALSE);
        }
    }
