...
#define GIRATINA_SHADOW_EVENT_EXT_COUNT 1
...
typedef struct DistWorldGiratinaShadowEventData {
    s16 initialTilePosX;
    s16 initialTilePosY;
    s16 initialTilePosZ;
...
} DistWorldGiratinaShadowEventData;
...
typedef struct DistWorldGiratinaShadowPropUserData {
    DistWorldSystem *system;
    DistWorldPropRenderer *renderer;
    DistWorldGiratinaShadowEventData eventData;
...
static BOOL DistWorldPropAnimInfo_HasNoAnimation(int propKind);
...
static void FreeGiratinaShadowPropRenderer(DistWorldSystem *param0);
static void InitGiratinaShadowPropAnimation(DistWorldSystem *param0, const DistWorldGiratinaShadowEventData *param1);
static BOOL IsGiratinaShadowAnimationFinished(DistWorldSystem *param0);
static void Dummy0224E984(DistWorldSystem *param0);
static void FinishGiratinaShadowPropRenderer(DistWorldSystem *param0);
static BOOL IsGiratinaShadowPropRendererValid2(DistWorldSystem *param0, u32 param1);
static BOOL IsGiratinaShadowPropRendererAnimValid(DistWorldSystem *param0, u32 param1);
static BOOL IsGiratinaShadowPropRendererValid(DistWorldSystem *param0, int param1);
...
static void PlaySoundEffect(u16 seqID);
...
static BOOL DistWorldPlatformProp_AnimInit(OverworldAnimManager *animMan, void *context)
{
    ...
    VecFx32_FromMapObjectPos(ghostPropTemplate->tileX, ghostPropTemplate->tileZ, &platformProp->initialPos);
    ...
    if (ghostPropHidden == TRUE) {
        if (platformProp->opacity > GHOST_PROP_OPACITY_MIN) {
            ...
            if (platformProp->soundEffectState != PLATFORM_PROP_SFX_STATE_DISAPPEAR) {
                ...
                PlaySoundEffect(SEQ_SE_PL_SYUWA3);
            }
        }
...
static BOOL DistWorldObstacleProp_AnimInit(OverworldAnimManager *animMan, void *context)
{
    ...
    VecFx32_FromMapObjectPos(ghostPropTemplate->tileX, ghostPropTemplate->tileZ, &obstacleProp->pos);
    ...
    if (ghostPropGroupHidden == TRUE) {
        ...
        if (obstacleProp->opacity >= animFrame / FX32_ONE) {
            if (obstacleProp->opacity > GHOST_PROP_OPACITY_MIN) {
                ...
                if (obstacleProp->soundEffectState != OBSTACLE_PROP_SFX_STATE_DISAPPEAR) {
                    ...
                    PlaySoundEffect(obstacleProp->seqIDDisappear);
                }
        ...
        if (obstacleProp->opacity < GHOST_PROP_OPACITY_MAX) {
            ...
            if (obstacleProp->soundEffectState != OBSTACLE_PROP_SFX_STATE_APPEAR) {
                ...
                PlaySoundEffect(obstacleProp->seqIDAppear);
            }
...
static DistWorldPropRenderer *DistWorldPropRenderer_Init(DistWorldSystem *system, u32 propKind, BOOL *alreadyInit)
{
    ...
    if (DistWorldPropAnimInfo_HasNoAnimation(propKind) == TRUE) {
...
static void DistWorldPropRenderer_InvalidateAnimated(DistWorldSystem *system, DistWorldPropRenderer *propRenderer)
{
    if (!DistWorldPropAnimInfo_HasNoAnimation(propRenderer->propKind)) {
...
{
    const DistWorldPropAnimInfo *animInfo = &sPropAnimInfoByKind[propKind];
    return animInfo->hasNoAnimation == TRUE;
}
...
static const DistWorldGiratinaShadowEventData sGiratinaShadowEventDataExt[GIRATINA_SHADOW_EVENT_EXT_COUNT] = {
    {
        .initialTilePosX = 0x6,
        .initialTilePosY = 0x129,
        .initialTilePosZ = 0x2A,
...
{
    GF_ASSERT(eventIndex < GIRATINA_SHADOW_EVENT_EXT_COUNT);

    DistWorldSystem *dwSystem = fieldSystem->unk_04->dynamicMapFeaturesData;
    InitGiratinaShadowPropAnimation(dwSystem, &sGiratinaShadowEventDataExt[eventIndex]);
}
...
static void InitGiratinaShadowPropRenderer(DistWorldSystem *system)
{
...
static void InitGiratinaShadowPropAnimation(DistWorldSystem *system, const DistWorldGiratinaShadowEventData *eventData)
{
    DistWorldGiratinaShadowPropUserData userData;
    DistWorldGiratinaShadowPropRenderer *giratinaRenderer = &system->giratinaShadowPropRenderer;

    InitGiratinaShadowPropRenderer(system);
...
static BOOL DistWorldGiratinaShadowProp_AnimInit(OverworldAnimManager *animMan, void *context)
{
    ...
    const DistWorldGiratinaShadowEventData *eventData;

    giratinaProp->userData = *userData;
    eventData = &giratinaProp->userData.eventData;

    giratinaProp->animStopped = TRUE;
    giratinaProp->rotation = sGiratinaShadowPropRotationAngles[eventData->rotationAnglesIndex];

    pos.x = (eventData->initialTilePosX << 4) * FX32_ONE;
    pos.y = (eventData->initialTilePosY << 4) * FX32_ONE;
    pos.z = (eventData->initialTilePosZ << 4) * FX32_ONE;

    if (eventData->rotationAnglesIndex == 4) {
        pos.x += FX32_ONE * 8;
    }
...
{
    DistWorldGiratinaShadowProp *giratinaProp = context;
    const DistWorldGiratinaShadowEventData *eventData = &giratinaProp->userData.eventData;

    switch (giratinaProp->state) {
    case GIRATINA_SHADOW_PROP_STATE_SFX: {
        if (eventData->soundKind == GIRATINA_SHADOW_PROP_SFX_KIND_CRY) {
            Sound_PlayPokemonCry(SPECIES_GIRATINA, 0);
        } else if (eventData->soundKind == GIRATINA_SHADOW_PROP_SFX_KIND_FLEE) {
    ...
    case GIRATINA_SHADOW_PROP_STATE_MOVE: {
        VecFx32 pos;
        const VecFx32 *posDelta = &eventData->posDelta;
        ...
        if (giratinaProp->currentMovementAnimStep >= eventData->movementAnimSteps) {
            giratinaProp->state++;
...
static void DistWorldGiratinaShadowProp_AnimRender(OverworldAnimManager *animMan, void *context)
{
    ...
    if (!giratinaProp->animStopped) {
        ...
        const VecFx32 *scale = &giratinaProp->userData.eventData.scale;
...
{
    const DistWorldGiratinaShadowEventData *v0 = param3;

    InitGiratinaShadowPropAnimation(param0, v0);
    *param2 = 1;
    return 0;
}
...
static const DistWorldGiratinaShadowEventData Unk_ov9_022523A8 = {
    .initialTilePosX = 0x3F,
    .initialTilePosY = 0xA9,
    .initialTilePosZ = 0x9,
...
static const DistWorldGiratinaShadowEventData Unk_ov9_022523CC = {
    .initialTilePosX = 0x2A,
    .initialTilePosY = 0x89,
    .initialTilePosZ = 0x20,
    .rotationAnglesIndex = 0x3,
    .soundKind = GIRATINA_SHADOW_PROP_SFX_KIND_CRY,
    .scale = { FX32_ONE, FX32_ONE, FX32_ONE },
    .posDelta = { (FX32_ONE * 48), 0x0, 0x0 },
...
static const DistWorldGiratinaShadowEventData Unk_ov9_02252438 = {
    .initialTilePosX = 0xFFFFFFFFFFFFFFF7,
    .initialTilePosY = -4,
    .initialTilePosZ = 0x16,
    .rotationAnglesIndex = 0x3,
...
static const DistWorldGiratinaShadowEventData Unk_ov9_0225245C = {
    .initialTilePosX = 0xF,
    .initialTilePosY = -34,
    .initialTilePosZ = 0x8,
