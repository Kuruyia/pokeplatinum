...
enum SkyboxKind {
    SKYBOX_NORMAL = 0,
    SKYBOX_GIRATINA_ROOM,
    SKYBOX_GIRATINA_ROOM_DARK,
    SKYBOX_KIND_COUNT,
};
...
typedef struct CmdParamsPersistedBoulderPuzzleFlag {
    u32 flagIndex;
} CmdParamsPersistedBoulderPuzzleFlag;

typedef struct CmdRunDataPlayGiratinaRoomAppearance {
    fx32 spriteDarkness;
    fx32 skyboxDarkness;
...
} CmdRunDataPlayGiratinaRoomAppearance;
...
typedef struct CmdRunDataShowGiratinaRoomPlatforms {
    s16 ghostPropGroup;
    s16 delay;
} CmdRunDataShowGiratinaRoomPlatforms;

typedef struct CmdRunDataHideGiratinaRoomPlatforms {
    s16 ghostPropGroup;
    s16 delay;
} CmdRunDataHideGiratinaRoomPlatforms;
...
{
    system->skyboxKind = SKYBOX_NORMAL;
    ...
    if (DistWorldSystem_GetMapHeaderID(system) == MAP_HEADER_DISTORTION_WORLD_GIRATINA_ROOM) {
        ...
        if (distWorldProgress >= 10) {
            if (distWorldProgress <= 12) {
                system->skyboxKind = SKYBOX_GIRATINA_ROOM;
            } else if (distWorldProgress <= 13) {
                system->skyboxKind = SKYBOX_GIRATINA_ROOM_DARK;
            }
        }
    }
...
{
    const CmdParamsPersistedBoulderPuzzleFlag *cmdParams = params;
    SetPersistedBoulderPuzzleFlag(system, cmdParams->flagIndex);

    return EVENT_CMD_HANDLER_RES_FINISH;
}

static const DistWorldEventCmdHandler sSetPersistedBoulderPuzzleFlagHandlers[] = {
...
static const DistWorldEventCmdHandler sClearPersistedBoulderPuzzleFlagHandlers[] = {
...
static void CmdRunDataPlayGiratinaRoomAppearance_UpdateSpritePalette(CmdRunDataPlayGiratinaRoomAppearance *runData)
{
...
void DistWorld_ApplyGiratinaSpritePalette(FieldSystem *fieldSystem)
{
    ...
    if (dwSystem->playingGiratinaRoomAppearance == TRUE) {
        CmdRunDataPlayGiratinaRoomAppearance *runData = GetLoadedEventDataBuffer(dwSystem);
...
{
    CmdRunDataPlayGiratinaRoomAppearance *runData = ResetLoadedEventDataBuffer(system, sizeof(CmdRunDataPlayGiratinaRoomAppearance));
...
{
    CmdRunDataPlayGiratinaRoomAppearance *runData = GetLoadedEventDataBuffer(system);
    ...
    if (v0 != NULL) {
        ...
        CmdRunDataPlayGiratinaRoomAppearance_UpdateSpritePalette(runData);
...
static const DistWorldEventCmdHandler sPlayGiratinaRoomAppearanceHandlers[] = {
...
{
    CmdRunDataShowGiratinaRoomPlatforms *runData = ResetLoadedEventDataBuffer(system, sizeof(CmdRunDataShowGiratinaRoomPlatforms));
...
{
    CmdRunDataShowGiratinaRoomPlatforms *runData = GetLoadedEventDataBuffer(system);
...
{
    CmdRunDataHideGiratinaRoomPlatforms *runData = ResetLoadedEventDataBuffer(system, sizeof(CmdRunDataHideGiratinaRoomPlatforms));
...
{
    CmdRunDataHideGiratinaRoomPlatforms *runData = GetLoadedEventDataBuffer(system);
...
static const DistWorldEventCmdHandler *sEventCmdHandlers[EVENT_CMD_KIND_COUNT] = {
    ...
    [EVENT_CMD_SET_PERSISTED_BOULDER_PUZZLE_FLAG] = sSetPersistedBoulderPuzzleFlagHandlers,
    [EVENT_CMD_CASCADE_DOWN] = sCascadeDownHandlers,
    [EVENT_CMD_PLAY_GIRATINA_ROOM_APPEARANCE] = sPlayGiratinaRoomAppearanceHandlers,
    ...
    [EVENT_CMD_CLEAR_PERSISTED_BOULDER_PUZZLE_FLAG] = sClearPersistedBoulderPuzzleFlagHandlers
};
...
static const CmdParamsPersistedBoulderPuzzleFlag sUnkMapEventCmdParamsGiratinaRoom_1_1 = {
...
static const CmdParamsPersistedBoulderPuzzleFlag sUnkMapEventCmdParamsGiratinaRoom_2_1 = {
    .flagIndex = DIST_WORLD_PERSISTED_BOULDER_PUZZLE_FLAG_GIRATINA_ROOM_PLATFORMS_VISIBLE
};
