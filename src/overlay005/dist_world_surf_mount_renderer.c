...
static void GetFixedRotationAngles(enum AvatarDistortionState avatarDistortionState, int dir, Simple3DRotationAngles *outRotAngles);
...
static BOOL DistWorldSurfMountRenderer_AnimInit(OverworldAnimManager *animMan, void *context)
{
    ...
    if (OverworldAnimManager_GetID(animMan) == 1) {
        renderer->flags |= DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_TICK;
    }
...
static void DistWorldSurfMountRenderer_AnimTick(OverworldAnimManager *animMan, void *context)
{
    ...
    if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_TICK) == 0) {
        return;
    }
    ...
    if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_ROTATION_RECALCULATION) == 0) {
        GetFixedRotationAngles(renderer->avatarDistortionState, renderer->dir, &renderer->rotAngles);
    }

    if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_ORBIT_RECALCULATION) == 0) {
        renderer->orbitAngle = sOrbitAngles[renderer->avatarDistortionState];
    }

    if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_POSITION_RECALCULATION) == 0) {
        VecFx32 pos = sSpriteBasePosOffsets[renderer->avatarDistortionState];

        if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_USE_EXTERNAL_SPRITE_POS_OFFSET) == 0) {
...
static void DistWorldSurfMountRenderer_AnimRender(OverworldAnimManager *animMan, void *context)
{
    ...
    if ((renderer->flags & DIST_WORLD_SURF_MOUNT_RENDERER_FLAG_MASK_NO_RENDER) == 0) {
        VecFx32 pos;
