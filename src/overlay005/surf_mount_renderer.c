...
OverworldAnimManager *SurfMountRenderer_HandleSurfBegin(MapObject *surfMountMapObj, int tileX, int tileZ, int dir, BOOL reuseMapObjPos)
{
    ...
    if (!reuseMapObjPos) {
        ...
        VecFx32 mapObjPosOffset = { MAP_OBJECT_BASE_POS_OFFSET_X, MAP_OBJECT_BASE_POS_OFFSET_Y, MAP_OBJECT_BASE_POS_OFFSET_Z };

        MapObject_GetPosPtr(surfMountMapObj, &mapObjPos);
        MapObject_SetSpritePosOffset(surfMountMapObj, &mapObjPosOffset);
    }
...
static void SurfMountRenderer_AnimTick(OverworldAnimManager *animMan, void *context)
{
    ...
    if (sub_02062764(surfMountMapObj, renderer->mapObjLocalID, renderer->mapHeaderID) == FALSE) {
    ...
    VecFx32 mapObjPosOffset;
    mapObjPosOffset.x = MAP_OBJECT_BASE_POS_OFFSET_X;
    mapObjPosOffset.y = MAP_OBJECT_BASE_POS_OFFSET_Y + renderer->yOffset;
    mapObjPosOffset.z = MAP_OBJECT_BASE_POS_OFFSET_Z;
